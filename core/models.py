...
class StoreConfig(models.Model):
    ...
    address = models.TextField("Endereço completo", default="Lago Sul / Altiplano Leste\nBrasília — DF")
    maps_url = models.URLField("Link Google Maps", blank=True, help_text="Cole o link de compartilhamento do Google Maps")
    opening_hours = models.TextField("Horário de funcionamento", default="Seg à Sex: 8h às 17h\nSábado: 8h às 16h")
    delivery_text = models.CharField("Destaque da entrega (navbar/hero)", max_length=200, default="Pet shop premium no Lago Sul — Brasília/DF")
    ...
    about_image = models.ImageField("Imagem — Sobre a In Dog", upload_to="site/about/", blank=True, null=True)
    hero_title = models.CharField("Título principal (Hero)", max_length=200, default="Cuidado que seu pet sente, amor que fica.")
...
class InteractionLog(models.Model):
    ...
    produto = models.ForeignKey("products.Product", verbose_name="Produto", on_delete=models.SET_NULL, null=True, blank=True, related_name="interacoes", db_index=True)
    servico = models.ForeignKey("services.Service", verbose_name="Serviço", on_delete=models.SET_NULL, null=True, blank=True, related_name="interacoes", db_index=True)
    nome_produto_snapshot = models.CharField("Nome do produto", max_length=200, blank=True)
    preco_produto_snapshot = models.DecimalField("Preço do produto", max_digits=10, decimal_places=2, null=True, blank=True)
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        indexes = [models.Index(fields=["tipo", "created_at"]), models.Index(fields=["produto", "created_at"]), models.Index(fields=["servico", "created_at"])]
